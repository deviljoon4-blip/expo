...
open class JSwiftRuntime {
   ...
   */
  public typealias SyncFunctionClosure = @Sendable (_ this: consuming JSwiftValue, _ arguments: UnsafeBufferPointer<JSwiftValue>) throws -> JSwiftValue
  ...
  public func createSyncFunction(_ name: String, _ fn: @escaping SyncFunctionClosure) -> JSwiftFunction {
    let hostFunction = expo.createHostFunction(pointee, name) { runtime, this, arguments, count in
      ...
      do {
        ...
        // TODO: Implement throwing `jsi.JSError`, returns `undefined` until then
        return .undefined()
      }
    }
...
//      return facebook.react.createPromiseAsJSIValue(pointee, promiseSetup)
   ...
   */
  public func schedule(priority: SchedulerPriority = .normal, @_implicitSelfCapture _ closure: consuming @escaping () -> Void) {
